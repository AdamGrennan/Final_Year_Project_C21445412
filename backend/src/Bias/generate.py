...
def generate_diverse_samples(label, num_samples=41):
                ...
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that generates diverse and concise examples for bias detection."},
                    {
                        ...
                        "content": (
                            ...
                            "Do in first person so user talks about themselves, Make it unique, creative, and realistic. Only output the sentence, no explanations."
                        )
                    }
                ],
                max_tokens=80,
...
label_to_generate = "OverconfidenceBias"  # Change this to your desired label type
num_samples = 41  # Number of sentences to generate
...
df.to_csv(output_file_path, index=False,  mode='a', header=False, quoting=csv.QUOTE_MINIMAL)
