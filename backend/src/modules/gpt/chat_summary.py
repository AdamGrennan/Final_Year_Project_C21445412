    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Summarize decision-making processes based on user conversations, highlighting reasoning and detected biases/noise."
                },
                {
                    "role": "user",
                    "content": prompt
        ...
        summary = (
            f"Title: {title}\nTheme: {theme}\n"
            f"Biases: {', '.join(detected_bias) or 'None'}\n"
            f"Noise: {', '.join(detected_noise) or 'None'}\n\n"
            "Summary: GPT response failed due to an error."
        )

    chat_summary_response["chat_summary"] = summary
